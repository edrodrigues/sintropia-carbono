...
2. Executar contra o Supabase via `supabase/seed.sql` ou SQL editor
3. Verificar: cada tabela deve conter a região correta, ranking sequencial, dados de volume e delta
...
#### Ação:
Para cada chart:
1. Extrair Top N do CSV correspondente
...
4. Copiar para `public/dados/dados.json` (se existir)
...
#### Ação:
Reescrever as 4 seções afetadas e atualizar as datas de referência.

---

### 3.5. Atualizar Página de Certificadoras (hardcoded)
...
| # | Arquivo | Tipo de Edição | Prioridade |
|---|---|---|---|
...
1. **Rodar build** → `npm run build` (check por erros de tipo)
2. **Verificar ranking pages** → `/carbono/ranking-brasil`, `/carbono/ranking-mundo`, `/energia/ranking-brasil`, `/energia/ranking-mundo`
   - Confirmar 50 registros em cada
   - Confirmar novos valores de volume e delta
3. **Verificar charts** → As barras e gráficos de setor devem refletir os novos dados
4. **Verificar dados.json** → Download deve conter 50 empresas por seção
5. **Verificar dados.md** → Tabelas e datas atualizadas

---

## 6. Observações Importantes

- **Volume 2026 (YTD):** Os novos CSVs incluem dados parciais de 2026. A interface `CarbonStakeholder` já suporta `volume_2026`, mas as páginas de ranking atualmente não exibem esta coluna. Decidir se querem adicionar a coluna "2026 YTD" na tabela de ranking.
- **IRecs - Top 50 Brazil:** Diferentemente do dado atual (focado em compradores), o novo CSV lista **geradores/comercializadores** (AXIA, Voltalia, etc.). O papel (`papel_mercado`) e a abordagem mudaram significativamente — planejar como apresentar isso na UI.
- **IRecs - Top 50 World:** O dado atual está em MWh (energia renovável). O novo CSV (`IRECs - Top 50 World.csv`) parece conter dados em milhões de MWh ou TWh (ex.: Amazon: 28.5 → 34.2). Confirmar unidade antes de popular o banco.
- **Datas de atualização:** Atualizar todas as ocorrências de "Última atualização" nas páginas para refletir Junho 2026.

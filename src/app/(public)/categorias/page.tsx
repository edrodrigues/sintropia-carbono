import Link from "next/link";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import { Metadata } from "next";

export const metadata: Metadata = {
  title: "Categorias | Sintropia",
  description: "Explore todas as categorias de inteligência de mercado disponíveis na plataforma Sintropia.",
};

const categories = [
  {
    title: "Carbono Brasil",
    description: "Análisis e insights do mercado de carbono no Brasil",
    image: "https://images.unsplash.com/photo-1542273917363-3b1817f69a2d?q=80&w=600&auto=format&fit=crop",
    href: "/carbono-brasil",
    active: true,
  },
  {
    title: "Energia Brasil",
    description: "Mercado de energia renovável e certificados I-REC no Brasil",
    image: "https://images.unsplash.com/photo-1466611653911-95081537e5b7?q=80&w=600&auto=format&fit=crop",
    href: "/irec-brasil",
  },
  {
    title: "Carbono Mundo",
    description: "Mercado global de créditos de carbono",
    image: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=600&auto=format&fit=crop",
    href: "/carbono-mundo",
  },
  {
    title: "Preços Energia",
    description: "Análisis comparada de preços de certificados de energia",
    image: "https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?q=80&w=600&auto=format&fit=crop",
    href: "/irec-precos",
  },
  {
    title: "Projetos Carbono",
    description: "Dados sobre 7700+ projetos de carbono",
    image: "https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=600&auto=format&fit=crop",
    href: "/carbono-projetos",
  },
  {
    title: "Certificadoras",
    description: "Principais certificadoras de carbono e energia",
    image: "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?q=80&w=600&auto=format&fit=crop",
    href: "/certificadoras",
  },
  {
    title: "I-REC Mundo",
    description: "Mercado global de certificados I-REC",
    image: "https://images.unsplash.com/photo-1532601224476-15c79f2f7a51?q=80&w=600&auto=format&fit=crop",
    href: "/irec-mundo",
  },
  {
    title: "Preços Carbono",
    description: "Evolução e análisis de preços de carbono",
    image: "https://images.unsplash.com/photo-1569163139599-0f4517e36f51?q=80&w=600&auto=format&fit=crop",
    href: "/carbono-precos",
  },
];

export default function CategoriasPage() {
  return (
    <>
      <Header />
      <main id="main-content" className="min-h-screen bg-white dark:bg-gray-950">
        <div className="max-w-7xl mx-auto px-8 lg:px-16 py-16">
          <div className="mb-12">
            <h1 className="text-4xl font-bold text-forest-green mb-4">
              Categorias
            </h1>
            <p className="text-slate-500 text-lg max-w-2xl">
              Explore todas as categorias de inteligência de mercado disponíveis na plataforma Sintropia.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category, index) => (
              <Link
                key={index}
                href={category.href}
                className="group relative aspect-[4/3] rounded-2xl overflow-hidden block"
              >
                <img
                  src={category.image}
                  alt={category.title}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <span className="inline-block px-3 py-1 bg-forest-green text-white text-xs font-bold rounded-full mb-3">
                    {category.title}
                  </span>
                  <p className="text-white/90 text-sm font-medium">
                    {category.description}
                  </p>
                </div>
                {category.active && (
                  <div className="absolute top-4 right-4">
                    <span className="flex h-3 w-3">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span>
                    </span>
                  </div>
                )}
              </Link>
            ))}
          </div>
        </div>
      </main>
      <Footer />
    </>
  );
}

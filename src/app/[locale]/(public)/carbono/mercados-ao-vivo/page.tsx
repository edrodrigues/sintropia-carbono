...
export default async function CarbonoLiveMarketsPage({
  ...
  const filterOptions = {
    assetTypes: await getDistinctFilterValues("asset_type").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
    geographies: await getDistinctFilterValues("country").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
    registries: await getDistinctFilterValues("registry").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
    technologies: await getDistinctFilterValues("technology").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
    currencies: await getDistinctFilterValues("currency").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
    referenceTypes: await getDistinctFilterValues("reference_type").then((vals) =>
      vals.map((v) => ({ label: v, value: v }))
    ),
  };
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
        ...
        <div className="mt-6">
          ...
          {activeTab === "watchlist" && <WatchlistTab locale={locale} />}
        </div>
